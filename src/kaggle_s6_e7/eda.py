...
def missing_summary(train: pd.DataFrame, test: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["missing_rate_delta"] = (
        summary.test_missing_rate - summary.train_missing_rate
    )
...
def missingness_target_table(
    df: pd.DataFrame, col: str, target_col: str
) -> pd.DataFrame:
    """Compare class proportions for missing and observed rows."""
    return pd.crosstab(df[col].isna(), df[target_col], normalize="index").rename_axis(
        "is_missing"
    )


def numeric_shift_summary(
    train: pd.DataFrame, test: pd.DataFrame, cols: Iterable[str]
) -> pd.DataFrame:
        ...
        records.append(
            {
                ...
                "quantile_outlier_rate": (
                    (values < quantile_low) | (values > quantile_high)
                ).mean(),
            }
        )
